...
const PART_URL_RE = /^https:\/\/www\.acquisition\.gov\/far\/part-\d+$/i;
...
function main() {
  ...
  for (const q of questions) {
    ...
    if (!q.question || q.question.length < 20) errors.push(`${id}: question text is too short`);
    ...
    if (rich) {
      ...
      const partUrl = rich.farRefs?.part?.url;
      if (!partUrl || !PART_URL_RE.test(partUrl)) errors.push(`${id}: farRefs.part.url invalid or missing`);
    }
  }
